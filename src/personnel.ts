...
export async function addText(msg) {
    await msg.send(dedent`🍬 Заполните форму для отправки пользователю формы 🍬
...
export async function fracs(msg) {
    await msg.send(dedent`🍬 Список фракций 🍬
...
export async function form(msg) {
    ...
    await msg.send(dedent`🍬 Заполните форму для назначения на руководящий пост 🍬
...
user.hear(/^ВК кандидата: (.*)/i, async msg => {
    ...
    try {
        ...
        if (!id) return await msg.send('🚫 Введите корректный тег 🚫')
        let frac = msg.text.split('Фракция: ')[1].split('\n')[0].trim()
        if (!frac) return await msg.send('🚫 Введите id фракции /fracks! 🚫')
        let status = msg.text.split('Должность: ')[1].split('\n')[0].trim()
        if (!status) return await msg.send('🚫 Введите должность! 🚫')
        ...
        if (user) return await msg.send('🚫 Данный пользователь уже является кандидатом! 🚫')
        ...
        await msg.send(`🚫 | Произошла ошибка! Проверьте правильность введенных данных!`)
    }
})
...
user.hear(/^Игровой ник: (.*)/i, async msg => {
    ...
    try {
        ...
        if (!user) await msg.send(`🚫 | К сожалению вы не числитесь в списке будущих руководителей 💔`)
        else {
            let nick = msg.text.split('Игровой ник: ')[1].split('\n')[0].trim()
            if (!nick) return await msg.send('🚫 Введите корректный ник 🚫')
            let age = msg.text.split('Возраст: ')[1].split('\n')[0].trim()
            if (!age) return await msg.send('🚫 Не корректный возраст! 🚫')
            let type_add = msg.text.split('Тип назначения: ')[1].split('\n')[0].trim()
            if (!type_add) return await msg.send('🚫 Введите тип назначения! 🚫')
            let discord = msg.text.split('Дискорд: ')[1].split('\n')[0].trim()
            if (!discord) return await msg.send('🚫 Введите id Discord аккаунта! 🚫')
            let forum = msg.text.split('Форум: ')[1].split('\n')[0].trim()
            forum = await getShortURL(forum)
            const data = await getUserData(nick)
            if (data) return await msg.send('🚫 Данный ник уже занят! 🚫')
            if (!forum) return await msg.send('🚫 Введите url форума! 🚫')
            ...
            if (error) {
                console.error(`Logs » Не удалось добавить пользователя в базу данных!`)
                console.error(error)
                await msg.send(`🚫 Не удалось добавить вас в базу данных!\nОбратитесь к администрации!`)
            } else {
                ...
                if (error) {
                    ...
                    console.error(error)
                    await msg.send(`🚫 Не удалось удалить вас из базы данных!\nОбратитесь к администрации!`)
                } else {
...
export async function removedCandidate(msg, args) {
    if (!args) return await msg.send("🚫 Вы не указали ID пользователя! 🚫")
    let user = await getVkId(args[0])
    if (!user) user = (args[0])
    let data = await getUserData(user, "candidates")
    if (!data) {
        await msg.send(`🚫 | Данный человек и так не имеет доступа к форме 💔`)
    } else {
...
export async function recovery(msg, args, sender) {
    ...
    if (!user) user = args[0]
    let data = await getUserData(user)
    if (!data) return await msg.send({message: `🚫 Пользователь не найден в архиве! 🚫`})
    else if (data.access > 0) return await msg.send({message: `🚫 Пользователь не снят с должности! 🚫`})
    ...
    await saveUser(data)
    await msg.send({
        message: `${sender.rank} @id${msg.senderId} (${sender.nick}) восстановил${await getGender(msg.senderId)} @id${data.vk_id} (${data.nick})!`,
        disable_mentions: 1
    })
}